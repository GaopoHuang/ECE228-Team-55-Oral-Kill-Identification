...
def run(args):
    ...
    model, val_metrics, epoch_metrics = train(
        ref_only_df,
        cons_train_df,
        cons_val_df,
        label_encoder,
    ...
    def test_model(de_imgs_df, evaluator, dataset_name, run_metrics_list, predictions_dfs_list, rotate_aug=None):
        ...
        metrics_results['PR-curve'].savefig(os.path.join(save_path, pltname))
        ...
        print("Metrics result: ", metrics_results)
    ...
    epoch_metrics_df = pd.DataFrame(epoch_metrics, columns=['dataset', 'name', 'epoch', 'value'])
    ...
    for target_save_dir in [save_path, 'outputs']:
        ...
        run_metrics_df.to_csv(os.path.join(target_save_dir, 'run_metrics_{}'.format(os.path.basename(args.val_imgs_csv))))
        epoch_metrics_df.to_csv(os.path.join(target_save_dir, 'epoch_metrics_{}'.format(os.path.basename(args.val_imgs_csv))))

    torch.save(model.state_dict(), os.path.join(save_path, '{}.pth'.format(os.path.basename(args.val_imgs_csv))))

    # metrics = run.get_metrics()
    # run_metric_pickle_path = os.path.join(save_path, 'run_metrics.pkl')
    # with open(run_metric_pickle_path, 'wb') as f:
    #     pickle.dump(metrics, f)
    #     print("Successfully pickled run metrics into {}".format(run_metric_pickle_path))
