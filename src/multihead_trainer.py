...
def hneg_train_model(model, optimizer, scheduler,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch%30 ==0) and epoch!=0:
            torch.save(best_model_wts, os.path.join(results_dir, 'training_epoch_{}_.pth'.format(epoch)))
    ...
    def log_epoch_metrics(epoch_metrics):
        displayed_epoch_metrics = []
        for phase in ['train', 'val']:
            for k, v in epoch_metrics[phase].items(): 
                if len(v.history) != num_epochs:
                    print("The metric {} doesn NOT have data at every epoch".format(k))
                for e in range(len(v.history)):
                    displayed_epoch_metrics.append([phase, k, e, v.history[e]])
        return displayed_epoch_metrics
    epoch_metrics = log_epoch_metrics(epoch_metrics)

    return model, best_metrics, epoch_metrics
...
def train(ref_only_df,
    ...
    model, best_val_metrics, epoch_metrics = hneg_train_model(model, optimizer,
    ...
    return model, best_val_metrics, epoch_metrics
